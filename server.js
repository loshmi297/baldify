...
// ── Create a Replicate prediction (with retry on 429)
async function createPrediction(version, input) {
  ...
  throw new Error('Rate limit persists after retries. Please wait 1 minute and try again.');
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url === '/baldify') {
    try {
      ...
      console.log('  → Waiting 12s before inpainting call...');
      await sleep(12000);
